...
@export var tilt_speed: float = 50.0
@export var max_tilt_up: float = 35.0
@export var max_tilt_down: float = -5.0
...
func _ready() -> void:
	...
	_mouse_to_world = get_tree().get_first_node_in_group("world_position") as MouseToWorldPosition
...
func _process(delta: float) -> void:
	...
	_rotate_pan_towards(_mouse_to_world.global_hit_position, delta)
	_rotate_tilt_towards(_mouse_to_world.global_hit_position, delta)
...
func _rotate_pan_towards(global_target: Vector3, delta: float) -> void:
	var direction_to_target: Vector3 = global_target - pan.global_position
