...
func explode(explosion_force: float) -> void:
	...
	for child: RigidBody3D in get_children():
		...
		var force: Vector3 = direction * explosion_force + _random_inside_unit_sphere() * (explosion_force * 0.5)
		child.apply_central_force(force)
		child.apply_torque(_random_inside_unit_sphere() * explosion_force)
